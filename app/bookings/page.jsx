  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden group">
      ...
      {booking.addons && booking.addons.length > 0 && (
        <div className="px-4 py-3 border-t border-gray-50 bg-[#faf9f6]/50">
          <p className="text-[10px] font-semibold tracking-widest uppercase text-gray-400 mb-2">Experiences & Add-Ons</p>
          <div className="flex flex-wrap gap-2">
            {booking.addons.map((addon, i) => (
              <div key={i} className="inline-flex items-center gap-1.5 text-[11px] font-medium text-[#072720] bg-white border border-gray-100 px-2.5 py-1.5 rounded-lg">
                <svg className="w-3 h-3 text-[#C09A59] flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09z" />
                </svg>
                {addon.name}
                <span className="text-gray-400 font-normal">₹{(addon.price * (addon.quantity || 1)).toLocaleString('en-IN')}</span>
              </div>
            ))}
          </div>
        </div>
