...
export default function AdminCalendarPage() {
  ...
  const fetchDates = async () => {
    ...
    try {
      ...
      // This will still go through Next.js proxy as it's not /api/admin/
      const data = await apiFetch(`/api/villas/${villaSlug}/unavailable-dates`);
      setUnavailableDates(data?.data || []);

      // Fetch admin-only blocked dates list
      // This will bypass Next.js proxy and hit Go directly
      const adminData = await apiFetch(`/api/admin/block-dates?villaId=${selectedVilla}`);
  ...
  const handleBlockDates = async () => {
    ...
    try {
      await apiFetch('/api/admin/block-dates', {
        method: 'POST',
        body: JSON.stringify({
      ...
      showToast(e.message || 'Failed to block dates', 'error');
    }
    setSelecting(null);
    setBlockReason('');
    setShowModal(false);
  ...
  const handleUnblock = async (id) => {
    try {
      await apiFetch(`/api/admin/block-dates?id=${id}`, { method: 'DELETE' });
      ...
      <div className="min-h-screen bg-black text-white p-4 md:p-8">
        ...
        <header className="mb-10 flex flex-col md:flex-row md:items-end md:justify-between gap-6">
          <div>
            <h1 className="text-4xl font-black mb-2 flex items-center gap-3">
              <span className="text-amber-500 text-3xl">📅</span> Admin Calendar
            </h1>
            <p className="text-zinc-500 font-medium">Block dates and manage availability.</p>
          </div>
          
          <select 
            className="bg-zinc-900 border-2 border-zinc-800 text-white px-6 py-3 rounded-2xl font-bold focus:border-amber-500 transition-all outline-none"
            value={selectedVilla}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-[#111] border border-white/10 rounded-2xl p-6">
            <h3 className="text-lg font-semibold text-white mb-4">Blocked Dates List</h3>
            {loading ? (
