...
export default function AdminPricingPage() {
  ...
  const fetchPricing = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/admin/pricing?villaId=${selectedVilla}`);
      if (res.ok) {
  ...
  const handleUpdateBasePrice = async () => {
    ...
    try {
      const res = await fetch(`/api/admin/villas/${selectedVilla}/price`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-bold text-white">Dynamic Pricing</h1>
      ...
      <div className="bg-[#111] border border-white/10 rounded-2xl p-6 mb-6">
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <p className="text-white/40 text-xs">Active Overrides</p>
