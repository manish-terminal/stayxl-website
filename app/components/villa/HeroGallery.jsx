  ...
  return (
    <>
      <section className="relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 md:grid-rows-2 gap-1.5 md:rounded-2xl overflow-hidden md:h-[480px]">
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-5 md:hidden">
                <h1 className="text-xl font-light text-white mb-1">{name}</h1>
        ...
        {/* View Photos Button (floating bottom-right on desktop) */}
        <div className="hidden md:block absolute bottom-6 right-10 z-10">
          <button
            onClick={() => openModal(0)}
            className="flex items-center gap-2 px-4 py-2 bg-white/90 backdrop-blur-sm rounded-lg shadow-md hover:bg-white hover:shadow-lg transition-all duration-300 text-[#072720]"
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 6A2.25 2.25 0 016 3.75h2.25A2.25 2.25 0 0110.5 6v2.25a2.25 2.25 0 01-2.25 2.25H6a2.25 2.25 0 01-2.25-2.25V6zM3.75 15.75A2.25 2.25 0 016 13.5h2.25a2.25 2.25 0 012.25 2.25V18a2.25 2.25 0 01-2.25 2.25H6A2.25 2.25 0 013.75 18v-2.25zM13.5 6a2.25 2.25 0 012.25-2.25H18A2.25 2.25 0 0120.25 6v2.25A2.25 2.25 0 0118 10.5h-2.25a2.25 2.25 0 01-2.25-2.25V6zM13.5 15.75a2.25 2.25 0 012.25-2.25H18a2.25 2.25 0 012.25 2.25V18A2.25 2.25 0 0118 20.25h-2.25A2.25 2.25 0 0113.5 18v-2.25z" />
            </svg>
            <span className="text-xs font-medium tracking-wide">View all photos</span>
          </button>
        </div>
        ...
        <div className="hidden md:block max-w-7xl mx-auto px-6 lg:px-8 pt-6 pb-2">
          <div className="flex items-start justify-between">
            <div>
              <h1 className="text-2xl lg:text-3xl font-light text-[#072720] mb-1">{name}</h1>
        ...
        <div className="md:hidden px-4 py-3 flex gap-1.5 overflow-x-auto scrollbar-hide">
          ...
          {images.length > 5 && (
            <button
              onClick={() => openModal(0)}
              className="flex-shrink-0 w-14 h-10 rounded-lg bg-gray-100 flex items-center justify-center text-[10px] font-medium text-gray-500"
            >
              +{images.length - 5}
            </button>
          )}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] bg-white overflow-y-auto">
          {/* Sticky Header */}
          <div className="sticky top-0 z-10 bg-white/95 backdrop-blur-sm border-b border-gray-100 px-4 md:px-8 py-3 flex items-center justify-between">
          ...
          <div className="max-w-4xl mx-auto px-4 md:px-8 py-6 space-y-3">
            {images.map((img, i) => (
              <div key={i} className="relative w-full aspect-[16/10] rounded-xl overflow-hidden bg-gray-50">
                <Image
                  src={img}
                  alt={`${name} - ${i + 1}`}
                  fill
