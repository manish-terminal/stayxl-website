        ...
        <div className="flex-1 overflow-y-auto px-6 sm:px-10 py-8 space-y-8">
          ...
          <div className="text-center">
            ...
            <div className="flex items-center justify-center gap-1.5 text-sm text-gray-500">
              ...
              <span>{booking.villaLocation || booking.VillaLocation || booking.villa?.location}</span>
            </div>
          </div>
          ...
          <div className={`p-5 rounded-2xl border transition-all duration-300 ${booking.paymentMode === 'ADVANCE' ? 'bg-amber-50/50 border-amber-100' : 'bg-green-50/50 border-green-100'}`}>
            <div className="flex items-center justify-between mb-4">
              ...
              <span className={`text-base font-bold ${booking.paymentMode === 'ADVANCE' ? 'text-amber-900' : 'text-green-900'}`}>
                ₹{(booking.paymentMode === 'ADVANCE' ? (booking.advanceAmount || booking.AdvanceAmount || 0) : (booking.totalAmount || booking.TotalAmount || booking.total || 0)).toLocaleString('en-IN')}
              </span>
            </div>
            ...
            {booking.paymentMode === 'ADVANCE' && (
              <div className="space-y-3 pt-3 border-t border-amber-200/50">
                <div className="flex justify-between text-xs text-amber-700">
                  ...
                  <span className="font-semibold">₹{(booking.totalAmount || booking.TotalAmount || booking.total || 0).toLocaleString('en-IN')}</span>
                </div>
                <div className="bg-white/80 rounded-xl px-4 py-3 flex items-center gap-3">
                  ...
                  <p className="text-xs text-amber-800 leading-snug">
                    Remaining balance of <strong>₹{(booking.balanceAmount || booking.BalanceAmount || 0).toLocaleString('en-IN')}</strong> is due on check-in.
                  </p>
                </div>
              </div>
            )}
          ...
          <div className="flex items-center justify-between py-2 border-t border-gray-100">
            <div className="flex flex-col">
              ...
              <span className="text-sm font-mono text-[#072720] font-bold mt-1">#{(booking.id || booking.ID || '').slice(-8).toUpperCase()}</span>
            </div>
